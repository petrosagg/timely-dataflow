...
use futures_core::stream::Stream as NativeStream;
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        match shared_state.state {
            ...
            Poll::Pending => {
                ...
                // Poll::Ready(_)`.
                //
                // It's tempting to do this once rather than repeatedly cloning the waker each
                // time. However, the `TimelyFuture` can move between tasks on the executor, which
                // could cause a stale waker pointing to the wrong task, preventing `TimelyFuture`
                // from waking up correctly.
                //
                // TODO(petrosagg): it's possible to check for this using the `Waker::will_wake`
                // function
...
pub trait ToStreamAsync<T: Timestamp, D: Data> {
    ...
    /// }
    /// ```
    fn to_stream<S: Scope<Timestamp = T>>(self, scope: &mut S) -> (TimelyFuture, Stream<S, D>);
}

impl<T: Timestamp, D: Data, I: NativeStream<Item = Event<T, D>> + 'static> ToStreamAsync<T, D>
    for I
{
    fn to_stream<S: Scope<Timestamp = T>>(self, scope: &mut S) -> (TimelyFuture, Stream<S, D>) {
        ...
        // Closure state
        let mut source_stream = Box::pin(self);
